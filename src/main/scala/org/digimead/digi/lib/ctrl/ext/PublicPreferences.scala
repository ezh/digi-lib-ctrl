...
import org.digimead.digi.lib.ctrl.base.AppComponentEvent
...
object PublicPreferences extends Logging {
  @volatile private var preferences: PublicPreferences = null
  @volatile private var onSharedPreferenceChangeListeners: Seq[SharedPreferences.OnSharedPreferenceChangeListener] = Seq()
    ...
    location orElse getLocation(context) flatMap {
      file =>
        ...
        if (!publicPreferenceBlob.exists || publicPreferenceBlob.length == 0) {
          ...
          preferences = try {
            FileUtil.writeToStream(biStream, baoStream)
            val result = XSerializarion.unparcelFromArray[PublicPreferences](baoStream.toByteArray).getOrElse({
  ...
  def getLocation(context: Context, forceInternal: Boolean = false): Option[File] =
    CtrlUtil.getDirectory(context, ".", forceInternal, Some(true), Some(false), Some(true)).flatMap(dir => {
  ...
  def reset(context: Context) = synchronized {
    getLocation(context, false).foreach(FileUtil.deleteFile)
    getLocation(context, true).foreach(FileUtil.deleteFile)
    preferences = null
  }
  @Loggable
    ...
    try {
      Option(preferences).map {
        p =>
          ...
          val baoStream = new ByteArrayOutputStream()
          try {
            baoStream.write(XSerializarion.parcelToArray(p))
