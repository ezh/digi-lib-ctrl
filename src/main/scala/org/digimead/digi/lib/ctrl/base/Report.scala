...
import org.digimead.digi.lib.ctrl.storage.GoogleCloud
...
object Report extends Logging {
  ...
  def reportPrefix = "U" + android.os.Process.myUid + "-" + Util.dateFile(new Date()) + "-P" + android.os.Process.myPid
  @Loggable
              ...
              info.reportPathInternal.listFiles.filter(f => !f.getName.endsWith("dlog") && !f.getName.endsWith("dtrc")).foreach {
                fileFrom =>
                  if (fileFrom.length == 0) {
                    ...
                    if (FileUtil.copyFile(fileFrom, fileTo))
                      fileFrom.delete
                    else
                      fileTo.delete
                  }
