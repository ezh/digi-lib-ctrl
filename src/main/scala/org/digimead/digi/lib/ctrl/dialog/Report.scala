            ...
            AnyBase.info.get.foreach {
              info =>
                ...
                try {
                  ...
                  val file = new File(info.reportPathInternal, org.digimead.digi.lib.ctrl.base.Report.reportPrefix + ".description")
                  file.createNewFile()
                ...
                val submitResult = org.digimead.digi.lib.ctrl.base.Report.submit(activity, true, Some((f, n) => {
                ...
                if (submitResult)
                  org.digimead.digi.lib.ctrl.base.Report.cleanAfterReview()
                else {
  ...
  def takeScreenshot(activity: android.app.Activity) = AnyBase.info.get.foreach {
    info =>
      ...
      try {
        ...
        content.draw(canvas)
        val file = new File(info.reportPathInternal, org.digimead.digi.lib.ctrl.base.Report.reportPrefix + ".png")
        file.createNewFile()
  ...
  def searchAndSubmit(activity: Activity with DActivity) = AnyBase.info.get.map {
    info =>
      if (searchAndSubmitLock.compareAndSet(false, true)) {
        ...
        if (reports.exists(_.endsWith(org.digimead.digi.lib.ctrl.base.Report.traceFileExtension)))
