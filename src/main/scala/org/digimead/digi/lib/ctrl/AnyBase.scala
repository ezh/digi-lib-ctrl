...
import org.digimead.digi.lib.ctrl.log.AndroidRecordInit
import org.digimead.digi.lib.ctrl.log.AndroidLoggingInit
  ...
  object Info {
    def init(context: Context) = synchronized {
      if (AnyBase.info.get == None) {
        ...
        // 755
        val info = new AnyBase.Info(reportPathInternal = CtrlUtil.getDirectory(context, reportDirectory, true,
          Some(true), Some(false), Some(true)).get,
          // 755
          reportPathExternal = CtrlUtil.getDirectory(context, reportDirectory, false,
            Some(true), Some(false), Some(true)).get,
          appVersion = pi.versionName,
          appBuild = Util.dateString(new Date(pi.versionCode.toLong * 1000)),
